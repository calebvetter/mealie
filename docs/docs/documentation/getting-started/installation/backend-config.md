...
| TOKEN_TIME                    |          48           | The time in hours that a login/auth token is valid. Must be <= 9600 (400 days, in hours). Sessions slide: using Mealie renews the token for the same window, so an active user stays logged in. Checking "Remember Me" at login extends the session to 30 days when that is longer than `TOKEN_TIME`. |
